...
required_vars=(
  ...
  OCI_COMPARTMENT_OCID OCI_SUBNET_OCID OCI_IMAGE_OCID OCI_SSH_PUBLIC_KEY
  INSTANCE_NAME OCI_SHAPE OCI_OCPUS OCI_MEMORY_GB
)
...
METADATA="$(python3 - <<'PY'
import json, os
print(json.dumps({"ssh_authorized_keys": os.environ["OCI_SSH_PUBLIC_KEY"]}))
PY
)"
...
echo "::error::OCI returned an unexpected error."
exit 1
